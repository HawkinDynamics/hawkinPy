...
<!-- badges: start -->
![PyPI - Version](https://img.shields.io/pypi/v/hdforce)
...
HDFORCE provides simple functionality with Hawkin Dynamics API. These functions are for use with the Hawkin Dynamics API (spec v1.15+). You must be a Hawkin Dynamics user with an active integration account to utilize functions within the package.
...
* __authMethod__: Sets storage method of auth variables. One of env, file, manual, or keyring. Default to 'env'.
...
The authentication manager allows the user to manage authentication with various development strategies. The 'env' method was designed to allow for simple authentication while developing in a local or contained environment, where the refresh token and variables are stored in the system environment. The 'file' method can be used for more visible and dynamic interaction with the auth variables, as the refresh token and variables are stored with a .env file. While the 'manual' method allows for the use of the package without storing sensitive data, in so the refresh token is not stored and is only used for authentication. This could be helpful in the scenario of developing a password-protected application where the token can be used as a password. The 'keyring' method stores the refresh token in your operating system's credential store (macOS Keychain, Windows Credential Manager, or Linux Secret Service) via the [keyring](https://pypi.org/project/keyring/) package, so you can store it once and reconnect in later sessions without re-supplying the token — pass `refreshToken` the first time to store it, then omit it to read it back. Use a distinct `refreshToken_name` to keep separate profiles (e.g. multiple orgs).
...
# Run authentication manager with keyring method
