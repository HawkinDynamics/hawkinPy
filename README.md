...
In hdforce, we can create and manage your athentication settings with the `authManager` function. The arguments passed in `authManager` will be used to handle authentication for the other functions used in the program. With this funciton you can control these settings:
* __region__: Sets the url prefix of your cloud site
* __authMethod__: Sets storage method of auth variables. One of env, file, or manual. Defualt to 'env'.
* __refreshToken_name__: Provide or set name of refresh Token variable. Default to "HD_REFRESH_TOKEN".
* __refreshToken__: Optionally provided to pass token for authentication when authMethod="manual". Otherwise sets refresh token value when provided.
* __env_file_name__: Required when authManager="file". Directs storage location of variables.

The authentication manager allows the user to manage authentication with various development strategies. The 'env' method was designed to allow for simple authentication while developing in a local or contained environment, where the refresh tokena and variables are stored in system environment. The 'file' method can be used for more visible and dynamic interaction with the auth variables, as the refresh token and variables are stored with .env file. While the 'manual' method allows for use of the package without storing sensitive data, in so the refresh token is not stored and only used for authentication. This could be helpful in the scenario of developing a password protected application where the token can be used as a password.
...
``` Python
# Get Athletes
roster = hdforce.GetAthletes( inactive= False) # inactive is default to Fasle
